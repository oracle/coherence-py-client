...
class NSLookup:
    def __init__(self, host: str = "localhost", port: int = DEFAULT_PORT):
        ...
        self.port = port
        self.channel = bytes()
        # Create a dummy socket object to satisfy mypy
        # AF_INET: Address Family for IPv4
        # SOCK_STREAM: Socket Type for TCP
        self.conn = socket.socket(socket.AF_INET, socket.SOCK_STREAM)

    @staticmethod
    def open(host_port: str = "", timeout: int = DEFAULT_TIMEOUT) -> NSLookup:
        ns_lookup = NSLookup()
        if host_port:
            ...
            ns_lookup.host = "localhost"
            ns_lookup.port = DEFAULT_PORT

        ns_lookup.connect((ns_lookup.host, ns_lookup.port), timeout)
        return ns_lookup
    ...
    def discover_cluster_info(self) -> DiscoveredCluster:
        cluster = DiscoveredCluster()
        cluster.ns_port = self.port
        cluster.host = self.host

        cluster.cluster_name = self.lookup(CLUSTER_NAME_LOOKUP)
        cluster.management_urls = parse_results(self.lookup(NS_PREFIX + MANAGEMENT_LOOKUP))
        cluster.jmx_urls = parse_results(self.lookup(NS_PREFIX + JMX_LOOKUP))
        cluster.metrics_urls = parse_results(self.lookup(NS_PREFIX + METRICS_LOOKUP))
        cluster.grpc_proxy_endpoints = parse_results(self.lookup(NS_PREFIX + GRPC_PROXY_LOOKUP))

        return cluster

    def discover_name_service_ports(self) -> List[ClusterNSPort]:
        local_cluster = self.lookup(CLUSTER_NAME_LOOKUP)
        other_clusters = self.lookup(NS_PREFIX + CLUSTER_FOREIGN_LOOKUP)
        ...
        for cluster_ns_port in list_clusters[1:]:
            cluster_ns_port.port = int(
                self.lookup(f"{NS_PREFIX}{CLUSTER_FOREIGN_LOOKUP}/{cluster_ns_port.cluster_name}{NS_LOCAL_PORT}")
            )

        return list_clusters

    def lookup(self, name: str) -> str:
        response = self.lookup_internal(name)
        if len(response) <= 7:
            return ""
        return self.read_string(response)

    def lookup_internal(self, name: str) -> bytes:
        request = self.channel + NS_LOOKUP_REQ_ID
        request += self.write_packed_int(len(name)) + name.encode() + REQ_END_MARKER
        self.conn.sendall(self.write_packed_int(len(request)))
    ...
    def connect(self, address: Tuple[str, int], timeout: int = DEFAULT_TIMEOUT) -> None:
        self.conn = socket.create_connection(address, timeout)
        self.conn.setsockopt(socket.SOL_SOCKET, socket.SO_KEEPALIVE, 1)
        self.conn.setsockopt(socket.IPPROTO_TCP, socket.TCP_NODELAY, 1)
        self.conn.sendall(MULTIPLEXED_SOCKET)
        self.conn.sendall(NAME_SERVICE_SUB_PORT)
        self.conn.sendall(self.write_packed_int(len(CONNECTION_OPEN)))
        self.conn.sendall(CONNECTION_OPEN)
        self.conn.sendall(self.write_packed_int(len(CHANNEL_OPEN)))
        self.conn.sendall(CHANNEL_OPEN)
    ...
    def read_response(self) -> bytes:
        length, _ = self.read_packed_int(self.conn)
        data = self.conn.recv(length)
        return data

    @staticmethod
    def read_packed_int(r: socket.socket) -> Tuple[int, int]:
        value = ord(r.recv(1))
    ...
    def read_string(self, data: bytes) -> str:
        # length, pos = self.read_packed_int(bytes([data[6]]))
...
def main() -> None:
    ...
    try:
        nslookup = NSLookup.open("localhost:7574")
        print(f"Connected to {nslookup.get_host()}:{nslookup.get_port()}")

        # Test discovering cluster info
        cluster_info: DiscoveredCluster = nslookup.discover_cluster_info()
        ...
        # Test discovering name service ports
        ns_ports = nslookup.discover_name_service_ports()
